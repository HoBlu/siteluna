...
const features = [
	...
	},
	{
		...
		description:
			'Каждый домик оборудован всем необходимым для комфортного отдыха, включая Wi-Fi и современную технику.',
	...
	},
	{
		...
		description:
			'Мы следим за каждой мелочью — от свежего белья до уютных мелочей. Заботимся о вашем комфорте: преимущество аккумуляторная техника, которая не потревожит ваш отдых.',
...
export default function FeaturesSection() {
	return (
		<section className="py-20 px-4 bg-white hidden">
			<div className="max-w-7xl mx-auto">
					...
					{features.map((feature, index) => (
						...
						>
							<div className="neumorphism rounded-3xl p-8 h-full relative overflow-hidden">
								...
								{/* Decorative element */}
								<div
									className={`absolute -bottom-4 -right-4 w-24 h-24 bg-gradient-to-br ${feature.gradient} rounded-full opacity-5 group-hover:opacity-10 transition-opacity duration-500`}
								></div>
							</div>
						</motion.div>
					))}
				...
				>
					<p className="text-lg text-gray-700 mb-8 max-w-2xl mx-auto">
						Готовы испытать все эти удобства на себе? Забронируйте свой идеальный
						отдых уже сегодня!
					</p>
				</motion.div>
			</div>
